...
let optionInfos!: typeof OptionInfos
let queueWait!: typeof import('../utils/systemUtil').queueWait

type OptionGlobalTag = 'global'
type OptionOtherTag = 'protocol' | 'user'
type OptionAllTag = OptionGlobalTag | OptionOtherTag
export type OptionTag = OptionGlobalTag | OptionOtherTag | [OptionGlobalTag] | OptionOtherTag[]
...
function getOptionKey(name: string, config: OptionField<any>): string {
    ...
    if (tags.includes('protocol')) {
        if (!runtimeData.nowAdapter) throw new Error('当前没有适配器，无法获取协议配置项')
 ...
 * @param oldValue 旧值
 */
function runOnChangeHook<T>(option: OptionField<T>, newValue: T, oldValue: T): T|undefined {
...
export const OptionManager = {
     ...
     * @param data
     */
    async init(data: RunTimeDataElem) {
        // 加载依赖
        optionInfos = await import('./cfgs').then(mod => mod.OptionInfos)
        queueWait = await import('../utils/systemUtil').then(mod => mod.queueWait)
        // 迁移检测
        ...
        const isWhiteProp = (prop: any) => {
            ...
            if (['toString', 'then', 'catch', 'finally'].includes(prop)) return true
            if (prop.startsWith('__v')) return true
            return false
        }
        options = shallowReactive(new Proxy({} as AppConfig, {
            /**
             * 获取配置项
             * @param _
             * @param prop 配置文件项名称
            ...
            get(target: any, prop: any) {
                ...
                // 获取配置项对象
                const option = optionInfos[prop] as OptionField<any> | undefined
                if (!option)
                    throw new Error(`不存在的配置项 ${prop}`)
                // 获取配置项对象
            ...
            ownKeys(_: any) {
                return Reflect.ownKeys(optionInfos as any)
            }
        }))
        runtimeData = data
     ...
     * @returns
     */
    checkDefault(key: keyof AppConfig): boolean {
     ...
     * @param json
     */
    async loadAllFromString(json: string) {
        if (!initd) throw new Error('配置管理器未初始化')
     ...
     * 获取原始配置对象
     */
    get rawConfigs(): Record<string, any> {
        if (!initd) throw new Error('配置管理器未初始化')
        return rawConfigs
    },
