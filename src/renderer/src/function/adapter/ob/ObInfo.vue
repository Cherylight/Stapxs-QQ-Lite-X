<template>
    <div v-if="status != 'unknown'" :class="'bot-status ' + status">
        <div />
        <span>{{
            $t('连接_' + status, {
    ...
    <div class="bot-info">
        <div
            v-for="[key, value] in Object.entries(adapterInfo)"
            :key="'botinfo-' + key"
        >
            <span>
                <span>{{ key + ': ' }}</span>
...
const adapter = computed<OneBotAdapter>(() =>
    markRaw(runtimeData.nowAdapter as OneBotAdapter),
)
