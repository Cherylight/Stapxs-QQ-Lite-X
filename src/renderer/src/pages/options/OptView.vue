    ...
    <div class="opt-page">
        <div class="ss-card">
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('language'),
                    }"
                />
        ...
        <div class="ss-card">
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('opt_dark_mode'),
                    }"
                />
            ...
            <template v-if="!runtimeData.sysConfig.opt_auto_win_color">
                <div class="opt-item">
                    <div
                        :class="{
                            changed: !option.checkDefault('theme_color'),
                        }"
                    />
            ...
            <template v-if="backend.isDesktop() && browser.os != 'Linux'">
                <div class="opt-item">
                    <div
                        :class="{
                            changed: !option.checkDefault('opt_auto_win_color'),
                        }"
                    />
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('vibrancy'),
                    }"
                />
            ...
            <div v-if="runtimeData.sysConfig.vibrancy" class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('background_img'),
                    }"
                />
                ...
                <div class="file-choice">
                    ...
                    <div
                        v-if="runtimeData.sysConfig.background_img !== ''"
                        class="rm-btn"
            ...
            <div v-if="runtimeData.sysConfig.background_img" class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('background_img_blur'),
                    }"
                />
                ...
                <div class="ss-range">
                    <input
                        v-model="runtimeData.sysConfig.background_img_blur"
                        :style="`background-size: ${runtimeData.sysConfig.background_img_blur}% 100%;`"
                        type="range"
                        name="background_img_blur"
                    />
                    <span
                        :style="`color: var(--color-font${runtimeData.sysConfig.background_img_blur > 50 ? '-r' : ''})`"
                    >
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('auto_hide_side_bar'),
                    }"
                />
                ...
                <div>
                    <span>{{ $t('自动隐藏侧边栏') }}</span>
                    <span>{{
                        $t('emm...这里该写些什么东西好呢？不知道，摆（')
                    }}</span>
                </div>
                <div class="select-wrapper">
                    <select
                        v-model="runtimeData.sysConfig.auto_hide_side_bar"
                        name="auto_hide_side_bar"
                        title="auto_hide_side_bar"
                    >
                        <option value="none">
                            {{ $t('禁用（默认）') }}
                        </option>
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('side_bar_width'),
                    }"
                />
                ...
                <div>
                    <span>{{ $t('侧边栏宽度') }}</span>
                    <span>{{
                        $t('难以通过拖拽侧边栏调整时，可以用这个')
                    }}</span>
                </div>
                <div class="ss-range">
                    <input
                        v-model="runtimeData.sysConfig.side_bar_width"
                    ...
                    />
                    <span
                        :style="`color: var(--color-font${runtimeData.sysConfig.side_bar_width > 625 ? '-r' : ''})`"
                    >
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('hide_chat_head'),
                    }"
                />
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('hide_chat_bottom'),
                    }"
                />
        ...
        <div class="ss-card">
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('opt_fast_animation'),
                    }"
                />
                ...
                <div>
                    ...
                    <span>{{
                        $t(
                            '咻咻！此选项将使动画加速到 100ms 并去除部分浪费时间的组动画',
                        )
                    }}</span>
                </div>
                <Switch v-model="runtimeData.sysConfig.opt_fast_animation" />
            </div>
            <div v-if="isMobile() && !backend.isMobile()" class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('initial_scale'),
                    }"
                />
                ...
                <div class="ss-range">
                    ...
                    />
                    <span
                        :style="`color: var(--color-font${initialScaleShow / 0.05})`"
                    >
            ...
            <div v-if="isMobile() && !backend.isMobile()" class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('fs_adaptation'),
                    }"
                />
                ...
                <div class="ss-range">
                    ...
                    />
                    <span
                        :style="`color: var(--color-font${fsAdaptationShow / 50 > 0.5 ? '-r' : ''})`"
                    >
                        {{ fsAdaptationShow }} px
                    </span>
                </div>
            ...
            <div v-if="backend.isDesktop()" class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('opt_always_top'),
                    }"
                />
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('merge_forward_width'),
                    }"
                />
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('use_favicon_notice'),
                    }"
                />
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('use_super_face'),
                    }"
                />
                <font-awesome-icon :icon="['fas', 'face-laugh-squint']" />
                <div>
                    <span>{{ $t('超级表情') }}</span>
                    <span>{{ $t('小黄脸长大了，变成了大黄脸！') }}</span>
                </div>
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('hide_self_avatar'),
                    }"
                />
            ...
            <div class="opt-item">
                <div
                    :class="{
                        changed: !option.checkDefault('self_msg_direction'),
                    }"
                />
                ...
                <div class="select-wrapper">
                    <select
                        v-model="runtimeData.sysConfig.self_msg_direction"
                        name="self_msg_direction"
                        title="self_msg_direction"
                    >
                        <option value="left">
                            {{ $t('左边') }}
                        </option>
...
async function changeVibrancy() {
    if (!isVibrancy.value) {
        ...
        const re = await ensurePopBox(
            $t(
                '开启透明模式将会对性能产生较为明显的影响，建议不要在性能较差的设备上使用此功能；此功能可能会降低元素可读性。',
            ),
        )
        if (!re) {
...
// 一次性初始化一次缩放级别
watchEffect(() => {
    initialScaleShow.value = toRaw(runtimeData.sysConfig.initial_scale)
    fsAdaptationShow.value = toRaw(runtimeData.sysConfig.fs_adaptation)
})
// 获取当前使用的图标
...
function scaleSave() {
    ...
    // 保存提醒
    makeSureBoxId = textPopBox(
        $t('点击确认以应用缩放比例，预览将在 5 秒后取消……'),
        {
            svg: 'up-down-left-right',
            title: $t('确认缩放比例'),
            button: [
...
function getIconList() {
    const iconList = import.meta.glob('@renderer/assets/img/icons/*.png', {
        eager: true,
    })
    ...
    for (const key in iconList) {
        ...
        if (name || name === '') {
            if (!win.darkMode && !name.endsWith('Dark')) {
                iconListInfo.push({
                    name: name,
                    icon: (iconList[key] as any).default,
                })
...
function changeIcon(name: string) {
    backend.call('Onebot', 'changeIcon', false, {
        name: name != '' ? name + 'AppIcon' : name,
    })
    usedIcon.value = name
}
//#endregion
