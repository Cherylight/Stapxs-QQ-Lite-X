                ...
                >
                    ...
                    <template v-if="currentTool === 'rect'">
                        ...
                        <font-awesome-icon
                            class="rect-is-fill"
                            :icon="[
                                toolConfig.rect.fill ? 'fas' : 'far',
                                'square',
                            ]"
                            @click.stop="
                                toolConfig.rect.fill = !toolConfig.rect.fill
                            "
                        />
                    </template>
                </TransitionGroup>
                ...
                >
                    <div :key="currentImg?.src">
                        ...
                        >
                            <!-- 水平滚动条 -->
                            <div
                                v-hide="!showScrollbarX"
                                class="scrollbar x"
                                :class="{ dragging: scrollBarDrag === 'x' }"
                                @wheel.stop.prevent="
                                    onScrollbarWheel('x', $event)
                                "
                                @mousedown="onScrollbarDrag('x', $event)"
                            >
                                <div
                                    class="scrollbar-thumb"
                                    :style="scrollbarThumbXStyle"
                                />
                            </div>
                            <!-- 垂直滚动条 -->
                            <div
                                v-hide="!showScrollbarY"
                                class="scrollbar y"
                                :class="{ dragging: scrollBarDrag === 'y' }"
                                @wheel.stop.prevent="
                                    onScrollbarWheel('y', $event)
                                "
                                @mousedown.stop.prevent="
                                    onScrollbarDrag('y', $event)
                                "
                            >
                                <div
                                    class="scrollbar-thumb"
                                    :style="scrollbarThumbYStyle"
                                />
                            </div>
                            <img
                                v-show="!edit"
...
function autoFit() {
    ...
    if (info.height / info.width >= 3) {
        modify.scale =
            Math.min((vh.value * 100) / 2, info.width, vw.value * 90) /
            info.width
        modify.x = 0
...
function init() {
    ...
    if (runtimeData.tags.canCors) img.crossOrigin = 'anonymous'
...
const showScrollbarX = computed(() => {
    ...
    else return currentImgInfo.value.height * modify.scale > vw.value * 100
})
...
const scrollbarThumbXStyle = computed(() => {
    if (!currentImgInfo.value || loading.value)
        return { '--thumb': 0, '--pos': 0 }
...
function onScrollbarDrag(axis: 'x' | 'y', event: MouseEvent) {
    ...
    const updatePos = (event: MouseEvent) => {
        if (axis === 'x') lastPos = event.clientX
        else lastPos = event.clientY
    }
    updatePos(event)
    ...
    mousemoveMask(
        (event: MouseEvent) => {
            ...
            if (axis === 'x') {
                ...
                if (modify.rotate % 180 === 0) {
                    const maxOffset =
                        (info.width * modify.scale - vw.value * 100) / 2
                    modify.x = Math.max(
                        -maxOffset,
                        Math.min(modify.x, maxOffset),
                    )
                } else {
                    const maxOffset =
                        (info.height * modify.scale - vw.value * 100) / 2
                    modify.x = Math.max(
                        -maxOffset,
                        Math.min(modify.x, maxOffset),
                    )
                ...
                if (modify.rotate % 180 === 0) {
                    const maxOffset =
                        (info.height * modify.scale - vh.value * 100) / 2
                    modify.y = Math.max(
                        -maxOffset,
                        Math.min(modify.y, maxOffset),
                    )
                } else {
                    const maxOffset =
                        (info.width * modify.scale - vh.value * 100) / 2
                    modify.y = Math.max(
                        -maxOffset,
                        Math.min(modify.y, maxOffset),
                    )
...
function onWheel(event: WheelEvent) {
    ...
    if (event.deltaY > 0) newScale *= 0.9
    else newScale /= 0.9
...
function rectMouseUp(x: number, y: number) {
    ...
    if (toolConfig.rect.fill) ctx.fill()
    else ctx.stroke()
...
async function copyBlob(blob?: Blob) {
    ...
    await copyToClipboard([new window.ClipboardItem({ 'image/png': blob })])
    popInfo.info($t('复制成功'))
}
...
async function getBlob(): Promise<Blob | undefined> {
    return new Promise((resolve) => {
        let tmpUrl
        if (edit.value) tmpUrl = canvas.value!.toDataURL('image/png')
        else tmpUrl = currentImg.value?.src
        ...
        if (runtimeData.tags.canCors) tmpImg.crossOrigin = 'anonymous'
        tmpImg.src = tmpUrl
        tmpImg.onload = () => {
