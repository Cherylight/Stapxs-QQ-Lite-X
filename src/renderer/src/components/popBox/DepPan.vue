...
<template>
    <div class="main">
        <div v-if="type === undefined" class="ss-card power-by">
            <img src="@renderer/assets/img/Vue.png" alt="logo" class="logo" />
            <span style="color: var(--color-font-2); font-size: 0.8rem">{{
                $t('驱动自')
            }}</span>
        ...
        <div
            class="dept-list"
            :style="type === undefined ? '' : 'width: 100%;'"
        >
            <div v-if="type === undefined" class="ss-card info">
                <font-awesome-icon :icon="['fas', 'circle-info']" />
                <span id="deptLink">{{
                    $t(
                        '你可以在项目仓库的依赖关系图中找到大部分依赖，而这里列出了一些不由包管理管理的依赖。',
                    )
                }}</span>
            </div>
            <div v-else-if="type === 'service'" class="ss-card info">
                ...
                <span id="deptLink">{{
                    $t(
                        'Stapxs QQ Lite X 依赖的部分服务允许自行部署，此处只提供服务的来源；不保证所使用的服务来自官方。',
                    )
                }}</span>
            </div>
            ...
            <div v-if="type === 'service'" class="dept">
                ...
                <div
                    v-if="backend.platform == 'web'"
                    class="ss-card jump-card"
                    @click="
                        openLink('https://github.com/Stapxs/Stapxs-Web-API')
                    "
                >
                    <header>
                        <div />
            ...
            <div v-else class="dept">
                ...
                <div
                    ...
                    @click="
                        openLink(
                            'https://github.com/Stapxs/Border-Card-UI/tree/js-bcui',
                        )
                    "
                >
                    <header>
                ...
                <div
                    class="ss-card jump-card"
                    @click="
                        openLink('https://github.com/SAWARATSUKI/KawaiiLogos')
                    "
                >
                    <header>
                        <div />
