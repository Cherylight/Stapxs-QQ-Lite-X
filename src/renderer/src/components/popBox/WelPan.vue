...
<template>
    <div v-if="show != 'home'" class="step-line">
        <font-awesome-icon
            :class="show == 'home' ? 'select' : ''"
            :icon="['fas', 'earth-asia']"
            @click="changeView('home')"
        />
        <div />
        <font-awesome-icon
            :class="show == 'license' ? 'select' : ''"
            :icon="['fas', 'pen-nib']"
            @click="changeView('license')"
        />
        <div />
        <font-awesome-icon
            :class="show.startsWith('function') ? 'select' : ''"
            :icon="['fas', 'brush']"
            @click="changeView('function')"
        />
        <div />
        <font-awesome-icon
            :class="show.startsWith('tip') ? 'select' : ''"
            :icon="['fas', 'book']"
            @click="changeView('tip')"
        />
        <div />
        <font-awesome-icon
            :class="show.startsWith('info') ? 'select' : ''"
            :icon="['fas', 'circle-info']"
            @click="changeView('info')"
        />
        <div />
        <font-awesome-icon
            :class="show == 'end' ? 'select' : ''"
            :icon="['fas', 'circle-check']"
            @click="changeView('end')"
        />
    </div>
        ...
        <div class="lead">
            ...
            <div>
                ...
                <a
                    @click="
                        openLink(
                            'https://github.com/Chzxxuanzheng/Stapxs-QQ-Lite-X/blob/test/DISCLAIMER.md',
                            true,
                        )
                    "
                    >Stapxs QQ Lite X 免责条款（简体中文）</a
                >
                <a
                    @click="
                        openLink(
                            'https://github.com/Chzxxuanzheng/Stapxs-QQ-Lite-X/blob/test/LICENSE',
                            true,
                        )
                    "
                    >Stapxs QQ Lite 开源许可 X（英文）</a
                >
                <br />
    ...
    <div v-else-if="show == 'function'" class="function">
        <div class="config">
            ...
            <div>
                ...
                <a>{{
                    $t(
                        'Stapxs QQ Lite 拥有一个主题色，你可以选择一个主题色作为主要风格！如果你喜欢保持深色主题，也可以关闭自动深色模式自行选择。',
                    )
                }}</a>
                    ...
                    <div class="theme-color-col">
                        ...
                        >
                            <input
                                ...
                                :data-id="index"
                                :checked="
                                    runtimeData.sysConfig.theme_color === index
                                "
                            />
                            <div
                                :style="
                ...
                <div class="opt-item">
                    ...
                    <DarkModeSwitch
                        v-model="runtimeData.sysConfig.opt_dark_mode"
                    />
                </div>
            </div>
        </div>
    ...
    <div v-else-if="show == 'function_msg'" class="function">
        <div class="config">
            ...
            <div>
                ...
                <a>{{
                    $t(
                        '群收纳盒将所有的群消息收进一个单独的群消息列表内并提供实时置顶新消息的功能；你可以关闭它来控制群消息的直接通知选项。',
                    )
                }}</a>
                <div class="opt-item wel-opt-item">
                    <div>
    ...
    <div v-else-if="show == 'function_img'" class="function">
        <div class="config">
            ...
            <div>
                ...
                <a>{{
                    $t(
                        '图片发送框默认启用，所有待发送的图片都将发送在文本之前。如果想要图文混排发送，你需要关闭这个功能以使用纯文本发送模式。',
                    )
                }}</a>
                <div class="opt-item wel-opt-item">
                    <div>
    ...
    <div v-else-if="show == 'tip'" class="function">
        <div class="config">
            ...
            <div>
                ...
                <a>{{
                    $t(
                        'Stapxs QQ Lite 的部分功能包含在元素的菜单中。你可以右击（或长按）消息列表、头像、消息等元素展开菜单来使用更多功能！',
                    )
                }}</a>
            </div>
        </div>
    ...
    <div v-else-if="show == 'tip_input'" class="function">
        <div class="config">
            ...
            <div>
                ...
                <a>{{
                    $t(
                        '消息发送框除了输入内容以外，同时支持粘贴图片、at 群成员功能；不过不支持富媒体显示和多行输入。',
                    )
                }}</a>
            </div>
        </div>
    ...
    <div v-if="show == 'info'" class="base-box">
        <div class="lead">
            ...
            <div>
                <span>{{
                    $t(
                        'Stapxs QQ Lite X 会将部分使用数据上传到上游的 umami 服务器中用于了解用户使用情况以及制作一些有趣的统计信息。',
                    )
                }}</span>
                <span style="margin-bottom: 20px">{{
                    $t('如果你并不希望上传这些数据，可以选择关闭它。')
                }}</span>
                <div
                    class="opt-item wel-opt-item"
                    :style="
    ...
    <div v-if="show == 'info_free'" class="base-box">
        <div class="lead">
            ...
            <div style="align-items: flex-end">
                <span>{{
                    $t(
                        'Stapxs QQ Lite X 是一个开源免费的软件，这意味着没有任何激活、使用限制等付费功能；',
                    )
                }}</span>
                <span style="margin-bottom: 20px">{{
                    $t(
                        '如果你通过任何付费方式获取了 Stapxs QQ Lite X，请及时追回损失并酌情反馈。',
                    )
                }}</span>

                <span>{{
                    $t(
                        'Stapxs QQ Lite X 仅在 GitHub 上发布。由于提供了自行部署的方式，非官方版本请谨慎使用。',
                    )
                }}</span>
                <font-awesome-icon
                    style="
    ...
    <div v-else-if="show == 'end'" class="wel-end">
                ...
                <g>
                    ...
                    >
                        <g>
                            <path
                                style="fill: var(--color-font-1)"
                                d="M103.734,1068.388c-4.417,0.137-7.886,3.828-7.749,8.244c0.076,2.453,1.274,4.736,3.25,6.193c27.302,20.898,41.291,44.17,53.531,69c1.959,3.961,6.758,5.584,10.719,3.625s5.584-6.758,3.625-10.719c-12.643-25.648-28.357-51.818-58.157-74.625C107.471,1068.934,105.623,1068.327,103.734,1068.388z"
                            />
                            <path
                                style="fill: var(--color-font-1)"
                                d="M93.203,1208.7c-15.382,1.072-31.39,4.859-48.406,12.344c-4.047,1.777-5.887,6.5-4.109,10.547s6.5,5.887,10.547,4.109l0,0c30.981-13.627,56.35-13.629,83.03-7.531c4.279,1.102,8.641-1.475,9.742-5.754c1.101-4.279-1.475-8.641-5.754-9.742c-0.141-0.037-0.282-0.068-0.425-0.098C123.354,1209.266,108.587,1207.624,93.203,1208.7L93.203,1208.7z"
                            />
                            <path
                                style="fill: var(--color-font-1)"
                                d="M369.984,1201.263c-14.917-0.225-28.656,2.047-42.469,3.125c-4.41,0.336-7.712,4.184-7.376,8.592c0.336,4.41,4.183,7.713,8.592,7.377c29.392-2.293,50.901-8.68,83.813,7.219c3.904,2.068,8.747,0.582,10.816-3.322s0.582-8.746-3.322-10.816c-0.182-0.096-0.367-0.186-0.556-0.268c-18.512-8.943-34.582-11.68-49.499-11.906H369.984z"
                            />
                            <path
                                style="fill: var(--color-font-1)"
                                d="M360.016,1068.388c-1.796,0.014-3.535,0.629-4.938,1.75c-31.296,24.119-45.662,50.006-58.125,74.531c-2.002,3.943-0.428,8.764,3.516,10.766c3.944,2.002,8.764,0.428,10.766-3.516c12.293-24.191,24.757-46.9,53.594-69.125c3.538-2.648,4.259-7.662,1.611-11.199C364.926,1069.571,362.544,1068.382,360.016,1068.388z"
                            />
                            <path
                                style="fill: var(--color-font-1)"
                            ...
                            <path
                                style="fill: var(--color-font-1)"
                                d="M271.797,1316.294c-4.418,0.09-7.927,3.742-7.838,8.16c0.038,1.875,0.732,3.676,1.962,5.09c23.275,27.518,21.758,48.701,22.094,74.938c0.055,4.418,3.681,7.955,8.099,7.9c4.418-0.055,7.956-3.68,7.901-8.098c0-0.01,0-0.018,0-0.027c-0.336-25.648,0.416-53.975-25.875-85.063c-1.559-1.889-3.896-2.959-6.344-2.906L271.797,1316.294z"
                            />
                            <path
                                style="fill: var(--color-font-1)"
                                d="M144.173,1276.325c-0.718,0-1.433,0.096-2.125,0.287c-38.275,9.793-48.533,33.836-60.438,51.063c-2.626,3.555-1.873,8.564,1.681,11.189c3.554,2.625,8.564,1.873,11.189-1.682c0.1-0.135,0.195-0.273,0.286-0.414c13.403-19.395,17.99-36.146,51.25-44.656c4.297-1.029,6.946-5.348,5.916-9.645c-0.861-3.592-4.067-6.127-7.759-6.137V1276.325z"
                            />
                            <path
                                style="fill: var(--color-font-1)"
                                d="M200.266,1316.263c-2.917-0.059-5.635,1.473-7.093,4c-16.816,28.021-22.731,57.02-17,85.686c0.816,4.342,4.999,7.201,9.341,6.385s7.201-4.998,6.385-9.342c-0.012-0.064-0.026-0.131-0.04-0.197c-4.926-24.648-0.176-48.984,15.032-74.314c2.329-3.754,1.174-8.688-2.581-11.016C203.094,1316.708,201.697,1316.294,200.266,1316.263z"
                            />
                        </g>
                        <path
                            style="fill: var(--color-font-1)"
                            d="M58.743,1071.813c-2.153,0.059-4.193,0.982-5.656,2.563l-15.75,16.938l-21.25-9.094	c-4.061-1.742-8.765,0.137-10.507,4.197c-0.983,2.293-0.842,4.912,0.383,7.084l11.25,20.188l-15.219,17.406	c-2.918,3.318-2.593,8.373,0.726,11.291c1.877,1.65,4.418,2.328,6.868,1.834l22.656-4.469l11.875,19.844	c2.266,3.793,7.178,5.031,10.972,2.766c2.128-1.271,3.547-3.459,3.841-5.92l2.75-22.969l22.562-5.125	c4.307-0.986,6.999-5.277,6.012-9.586c-0.553-2.414-2.195-4.438-4.444-5.477l-20.968-9.719l2.062-23.031	c0.398-4.4-2.847-8.291-7.248-8.689c-0.302-0.025-0.605-0.037-0.909-0.029L58.743,1071.813z M48.867,1102.438l-0.464,5.313	c-0.315,3.369,1.524,6.574,4.594,8l4.813,2.219l-5.157,1.184c-3.302,0.742-5.78,3.482-6.187,6.844l-0.624,5.281l-2.718-4.563	c-1.735-2.91-5.116-4.412-8.438-3.748l-5.218,0.992l3.499-4c2.225-2.541,2.616-6.203,0.976-9.156l-2.594-4.656l4.907,2.094	c3.102,1.326,6.702,0.563,9-1.906L48.867,1102.438z"
                        />
                        <path
                            style="fill: var(--color-font-1)"
                            d="M354.898,1378.251c-1.342,0.037-2.652,0.412-3.811,1.088c-1.876,1.09-3.23,2.895-3.75,5	l-5.531,22.406l-23,2.344c-4.399,0.416-7.629,4.318-7.214,8.717c0.236,2.502,1.634,4.748,3.775,6.064l19.658,12.219	l-4.875,22.594c-0.932,4.32,1.814,8.576,6.134,9.508c2.418,0.521,4.941-0.104,6.834-1.695l17.688-14.906l20,11.625	c3.835,2.195,8.723,0.865,10.918-2.971c1.212-2.115,1.391-4.67,0.488-6.936l-8.688-21.469l17.219-15.375	c3.309-2.93,3.616-7.984,0.688-11.293c-1.659-1.875-4.099-2.865-6.595-2.676l-23.062,1.686l-9.344-21.188	c-1.316-2.963-4.29-4.838-7.531-4.75L354.898,1378.251z M357.274,1410.938l2.125,4.844c1.366,3.096,4.531,4.998,7.906,4.75	l5.282-0.367l-3.938,3.531c-2.521,2.248-3.358,5.838-2.093,8.969l2.03,4.969l-4.624-2.688c-2.92-1.707-6.6-1.395-9.189,0.783	l-4.062,3.438l1.12-5.219c0.713-3.291-0.71-6.676-3.562-8.469l-4.531-2.813l5.28-0.527c3.364-0.336,6.154-2.75,6.97-6.031	L357.274,1410.938z"
                        />
                    </g>
                    <path
                        style="fill: var(--color-font-1)"
                        d="M216.019,1396.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S216.019,1391.95,216.019,1396.368L216.019,1396.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M40.019,1300.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S40.019,1295.95,40.019,1300.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M128.019,1372.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S128.019,1367.95,128.019,1372.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M112.019,1164.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S112.019,1159.95,112.019,1164.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M352.019,1156.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S352.019,1151.95,352.019,1156.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M384.019,1244.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S384.019,1239.95,384.019,1244.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M328.019,1356.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S328.019,1351.95,328.019,1356.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M160.019,1076.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S160.019,1071.95,160.019,1076.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M224.019,1028.369c0,4.418,3.582,8,8,8s8-3.582,8-8c0-4.418-3.582-8-8-8S224.019,1023.951,224.019,1028.369z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M288.019,1076.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S288.019,1071.95,288.019,1076.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M72.019,1252.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S72.019,1247.95,72.019,1252.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M64.019,1356.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S64.019,1351.95,64.019,1356.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M16.019,1236.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S16.019,1231.95,16.019,1236.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M368.019,1060.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S368.019,1055.95,368.019,1060.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M432.019,1228.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S432.019,1223.95,432.019,1228.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M392.019,1356.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S392.019,1351.95,392.019,1356.368z"
                    />
                    <path
                        style="fill: var(--color-font-1)"
                        d="M176.019,1428.368c0,4.418,3.582,8,8,8s8-3.582,8-8s-3.582-8-8-8S176.019,1423.95,176.019,1428.368z"
                    />
                </g>
            </g>
        </svg>
        ...
        <a>{{
            $t(
                '该说的都说了 —— 那么就可以愉快的用啦（大声），如果遇到什么奇怪的问题，尽管来 GitHub 仓库问哦。',
            )
        }}</a>
